...
function generateUpgradeChoices() {
    ...
    if (availableWeapons.length > 0 && choices.length < 3) {
        ...
        choices.push({
            ...
            title: `${weaponConfig.name} 等级${currentLevel + 1}`,
            description: upgrade.description,
            icon: getWeaponIcon(weaponName),
            effect: upgrade.effect,
            value: upgrade.value,
            rarity: getUpgradeRarity(currentLevel)
        });
    }
    ...
    while (choices.length < 3 && availablePassives.length > 0) {
        ...
        choices.push({
            type: 'passive',
            name: passiveConfig.name,
            title: `${passiveConfig.name} 等级${currentLevel + 1}`,
            description: upgrade.description,
            icon: getPassiveIcon(passiveName),
            effect: upgrade.effect,
            value: upgrade.value,
            rarity: getUpgradeRarity(currentLevel),
            passiveType: passiveName
        });
    }
...
// 获取升级稀有度
function getUpgradeRarity(level) {
...
function applyWeaponUpgrade(choice) {
    const weaponName = choice.weaponName || choice.name;
...
function applyPassiveUpgrade(choice) {
    const passiveName = choice.passiveType || choice.name;
